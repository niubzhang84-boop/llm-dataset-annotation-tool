...
4. **轻量级工业化本地 RAG（检索增强生成）系统**：
   * 采用中文高精度文本嵌入模型 `text2vec-base-chinese` 对私有语料进行高维向量化转换。
   * 基于矩阵点积计算高效捞取语义背景资料，成功解决本地轻量模型的“幻觉”痛点。
   * 通过锁死 `temperature=0.0` 及设计“三大纪律”强约束 Prompt，实现了高指令遵循度（Instruction Following）的开卷考试闭环。
